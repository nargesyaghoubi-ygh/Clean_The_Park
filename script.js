...
let recycled = 0;
let started = false;
let totalTrash = trashCount;   // Total trash in the current level

const trashes = [];
// Variables to track mouse or touch position
...
function createTrash(num) {
    for (let i = 0; i < num; i++) {
        ...
        // Adjust trash size based on screen width and level
...
// Function to handle recycling a trash item
...
// Update touch position on mobile devices
...
function animateTrashes() {
    const speed = 0.07 + level * 0.09;  // Increase speed with level
...
// Random chaos function to shuffle trash occasionally
function chaos() {
